...
const Dashboard = () => {
  ...
  const fetchSweets = async () => {
    try {
      ...
      if (response.data && Array.isArray(response.data)) {
        ...
        console.log(`✅ Loaded ${response.data.length} sweets successfully`)
      } else {
      ...
      const errorMsg = err.response?.data?.detail || err.message || 'Failed to fetch sweets'
      setError(errorMsg)
  ...
  const handlePurchase = async (sweetId: number) => {
    try {
      ...
      showToast(`${sweet?.name || 'Item'} purchased successfully!`, 'success')
    } catch (err: any) {
  ...
  const handleAddToCart = (sweet: Sweet) => {
    if (sweet.quantity === 0) {
      showToast('This sweet is out of stock!', 'error')
      return
    }
    addToCart(sweet)
  ...
  const getCartQuantity = (sweetId: number) => {
      ...
      <div className="app">
        <header className="app-header">
          <div className="header-content">
            <h1 className="app-title">🍬 Sweet Shop</h1>
      ...
      <div className="container">
        ...
        {!error && sweets.length === 0 && (
          <div className="empty-state fade-in">
            ...
            <h2 className="empty-state-title">No Sweets Available</h2>
            <p className="empty-state-description">
              The database might be empty or there was an issue loading sweets.
            </p>
            ...
            <div className="sweets-grid">
              {filteredSweets.map((sweet, index) => (
                ...
                >
                  ...
                  <div className={`sweet-card-stock stock-badge ${
                    sweet.quantity === 0 ? 'out-of-stock' : 
                    sweet.quantity < 10 ? 'low-stock' : 'in-stock'
                  }`}>
                    <span>Stock:</span>
                    <strong>{sweet.quantity}</strong>
                  </div>
                    ...
                    <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                      ...
                      {getCartQuantity(sweet.id) > 0 && (
                        <span className="cart-quantity-badge">
                          {getCartQuantity(sweet.id)} in cart
                        </span>
                      )}
                    </div>
                    <button
                      ...
                      style={{ width: '100%' }}
                    >
                      {sweet.quantity === 0 ? 'Out of Stock' : '⚡ Order Now'}
                    </button>
                  </div>
                </div>
              ))}
